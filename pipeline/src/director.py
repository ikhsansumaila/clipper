...
def director():
    ...
    print(f"📄 File transkrip ditemukan: {transcript_path}")
    ...
    3. Pastikan momen tersebut memiliki awal dan akhir yang jelas.
    4. Batasi durasi potongan video antara {config.MIN_CLIP_DURATION} hingga {config.MAX_CLIP_DURATION} detik.
    5. Berikan judul potongan video yang menarik dan relevan dengan momen tersebut.
    6. Wajib balas HANYA hasil analisanya saja tidak perlu penjelasan panjang lebar, kamu hanya perlu mengembalikan data dalam format JSON murni tanpa markdown, Berikut adalah contoh struktur data transkrip beserta timestamp detiknya:
    {{"start": 12.5, "end": 45.0, "reason": "Karena bagian ini lucunya natural", "title": "Momen Lucu yang Bikin Ngakak"}}
    """

    # 2. Menyiapkan payload JSON
    payload = {
        "model": config.AI_AGENT_MODEL_NAME,
        "messages": [
            {
                "role": "user",
                "content": prompt,
    ...
    curl_cmd = [
        "curl", "-s", config.AI_AGENT_API_ENDPOINT,
        "-H", f"Authorization: Bearer {config.AI_AGENT_API_KEY}",
    ...
    if result.stdout.strip() == "":
        raise ValueError("Response dari AI kosong. Pastikan API key dan endpoint benar, dan coba lagi.")
